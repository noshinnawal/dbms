...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        $stmt = $pdo->prepare("UPDATE courses SET course_code = ?, course_name = ?, description = ?, credits = ?, department = ? WHERE course_id = ?");
        $stmt->execute([$code, $name, $desc, $credits, $dept, $course_id]);
...
<main class="p-8 flex-1">
    <header class="mb-10 flex items-center justify-between">
        <div>
            <h1 class="text-3xl font-bold text-on-surface">Edit Course</h1>
            <p class="text-on-surface-variant mt-1">Modify course details for the academic catalog.</p>
        </div>
        <a href="index.php" class="flex items-center gap-2 text-on-surface-variant hover:text-primary transition-colors">
    ...
    <form action="edit.php?id=<?php echo $course_id; ?>" method="POST" class="max-w-3xl bg-surface-container rounded-[32px] p-8 shadow-[12px_12px_24px_#dbe4eb,-12px_-12px_24px_#ffffff]">
        <div class="flex flex-col gap-6">
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div class="md:col-span-1">
                    ...
                    <input type="text" id="course_code" name="course_code" required placeholder="CS101" value="<?php echo htmlspecialchars($course['course_code']); ?>"
                ...
                <div class="md:col-span-2">
                    ...
                    <input type="text" id="course_name" name="course_name" required placeholder="Introduction to Computer Science" value="<?php echo htmlspecialchars($course['course_name']); ?>"
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <select id="department" name="department" required
                        ...
                        <option value="Computer Science" <?php echo $course['department'] == 'Computer Science' ? 'selected' : ''; ?>>Computer Science</option>
                        <option value="Mathematics" <?php echo $course['department'] == 'Mathematics' ? 'selected' : ''; ?>>Mathematics</option>
                        <option value="Physics" <?php echo $course['department'] == 'Physics' ? 'selected' : ''; ?>>Physics</option>
                        <option value="English" <?php echo $course['department'] == 'English' ? 'selected' : ''; ?>>English</option>
                        <option value="Business Administration" <?php echo $course['department'] == 'Business Administration' ? 'selected' : ''; ?>>Business Administration</option>
                    </select>
                </div>
                <div>
            ...
            <div>
                ...
                <textarea id="description" name="description" rows="4"
                          class="w-full bg-surface-container border-none rounded-2xl py-3 px-4 text-on-surface shadow-[inset_4px_4px_8px_#dbe4eb,inset_-4px_-4px_8px_#ffffff] outline-none focus:ring-1 focus:ring-tertiary/30 resize-none"><?php echo htmlspecialchars($course['description']); ?></textarea>
            </div>
